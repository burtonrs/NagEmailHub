...
# - Update NAGEMAIL testing variables with new columns
# - Update DEFAULT_SENDER to RIS-GLOAPPINF email address

DEFAULT_SENDER = os.getenv("DEFAULT_SENDER", "ris-gloappinf@risk.lexisnexis.com")
...
def send_email(recipient, message = "<b>Hello!</b>", sender = DEFAULT_SENDER, subject = "", server = EMAIL_SERVER, server_port = EMAIL_SERVER_PORT):
    ...
    try:
      with smtplib.SMTP(server, server_port) as smtp:
        ...
        logging.error("Error email may have not been sent: %s" % recipient)
...
infile = os.getenv("INFILE", "sslcertcheckpeek2_allurls_mysql.csv")
...
with open(infile, newline='') as csvfile:
    ...
    for row in reader:
        ...
        intexp = row.get("Internal Expiration", "")
        issuer = row.get("Issuer", "")
        days = row.get("Days", "")
        copy2customer = row.get("Copytocust", "")
        checked = row.get("Checked", "")
        supportedby = row.get("Supported by", "")
        if supportedby.isspace():
            supportedby = ""
        businessunit = row.get("BU", "")
...
for cert in sslcertlist[:]:
    ...
    thisintexp = cert['INTERNAL EXPIRATION']
    logging.debug("NAGEMAIL: expiration="+thisintexp)
    ...
    if (certexpnotice != ""):
        ...
        logging.info("Expiration Date is: "+str(thisintexp))
        ...
        subject_out = "SSL Certificate for "+thiscommonname+" Expires on "+str(thisintexp)+" - ["+str(thisdays)+" Days]"
    else:
    ...
    if not bademail:
        if _90day or _60day or _30day or _15day:
            ...
            if _15day or not (thiscommonname in sentlogarray):
                ...
                if thisinstaller.lower() == appinfemail.lower():
                    ...
                    message = data.format(expdate = thisintexp, url = thisurl, commonname = thiscommonname, ticket = thisticket)
                    #emailSentTo = emailSentTo+thisdays+" day email being sent to "+thissupportedby+" for Common Name "+thiscommonname+"\n"
                    emailSentTo = "%s%s day email being sent to %s for Common Name %s\n" % (emailSentTo, thisdays, thissupportedby, thiscommonname)
                else:
        ...
        invalidrecord = "<tr><td>"+thisurl+"</td><td>"+thiscommonname+"</td><td>"+str(thisintexp)+"</td><td>"+str(thisdays)+"</td><td>"+badaddr+"</td></tr>"
